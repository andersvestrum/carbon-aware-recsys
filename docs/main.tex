...
product metadata~\cite{amazon2015, amazon2023, vicenti2026pcfllm}. While such
...
already available~\cite{spillo2023sustainabilityaware, spillo2023greenerrec} or operates
...
combines semantic similarity search with structured LLM prompting and a nearest-neighbour
fallback to infer PCF values for Amazon products from available
...
strong alignment to SDG~12~\cite{koundurai2024greendeal}. The EU's Directive 2024/825
...
which imposes transparency and user-control requirements for recommendation systems operated
by very large online platforms. Research on algorithmic feedback loops also demonstrates
...
all-MiniLM-L6-v2 sentence encoder~\cite{reimers2019sbert} and retrieve the $k$ nearest
...
(\texttt{Qwen/Qwen2.5-3B-Instruct}) with the product title alone,
...
prompting~\cite{brown2020gpt3}. For each query product, we embed its title using
\texttt{all-MiniLM-L6-v2}~\cite{reimers2019sbert} and retrieve its five nearest Carbon
Catalogue neighbours by cosine similarity. These neighbours are assembled into a structured
prompt in which they serve as labelled in-context examples, as illustrated in
Figure~\ref{fig:llm-prompt}. The model is instructed to reason step by step before
...
downstream analysis. In the reported run, the full Amazon catalog is scored with the local
\texttt{Qwen/Qwen2.5-3B-Instruct} few-shot pipeline, so the selected PCF column is driven by local Qwen predictions.
...
interactions occur strictly after their training history. To make experimentation tractable
while preserving dataset diversity, we sample up to 1{,}000 users per category, yielding
approximately 6{,}400--6{,}550 interactions per category after filtering. PCF estimates are
assigned to items from a catalog of 72{,}000 scored Amazon products; items without a PCF
estimate are excluded from re-ranking.
...
different recommendation models. When $\lambda = 0$, ranking depends only on
model-predicted relevance; when $\lambda = 1$, ranking prioritizes lower-carbon items.
...
We report NDCG@10 and Recall@10, which measure the quality of the top-10 recommended items
using observed test interactions as ground truth.
...
grid and compute NDCG@10, Recall@10, and AvgPCF@10 for each setting. We then plot NDCG@10
...
We follow the pipeline described in Section~4. Each category is run with 1{,}000 sampled
users; candidate generation models are trained on the training split and scored on held-out
test users. All evaluation is performed on top-10 recommendation lists. $\lambda$ is swept
over a 25-point grid spanning $[0,1]$: fine steps of 0.025 for $\lambda \leq 0.1$, coarser
steps of 0.05--0.1 through $\lambda=0.8$, and dense steps of 0.01 from $\lambda=0.90$ to
$\lambda=1.00$ to capture the engagement cliff precisely.
...
Recall@10, AvgPCF@10, and carbon reduction relative to the $\lambda=0$ baseline. This
...
(seed fixed for reproducibility). For each item, the true PCF is hidden, the five nearest
labelled neighbours are retrieved, and predictions are generated by each method. Zero-shot
...
valid prediction ($n=771$), so every method is evaluated on the same rows. The full
866-item holdout remains a robustness check. The downstream pipeline uses 
...
\paragraph{Findings.}
On the consumer-scale subset, the few-shot LLM is the strongest standalone estimator on
absolute-error metrics, achieving the lowest RMSE (1{,}708.6), MAE (695.1), and median
absolute error (58.6), and we therefore adopt it as our primary PCF estimator. This choice
is motivated by the downstream re-ranking objective, which depends on well-calibrated
absolute PCF values. The neighbour-average baseline remains best on rank preservation
($\rho=0.728$), as it inherits the local ordering structure of the embedding space and
thus preserves relative similarities even when absolute values are biased. On the
intersection benchmark in Table~\ref{tab:pcf}, the repository's \emph{selected} PCF
coincides with few-shot LLM.

The full 866-item holdout tells a different story: there, the neighbour-average baseline
remains best on RMSE, MAE, and Spearman, while the LLM-based estimators are heavily
penalized by a small number of extreme industrial products. In the current split, 92 of the
866 held-out items have true PCF above 10{,}000~kg CO$_2$e, which lies beyond the
consumer-oriented clamp applied to the zero-shot and few-shot prompts. Those outliers
dominate full-holdout RMSE and make the raw aggregate table less representative of
downstream e-commerce performance. We therefore use the consumer-scale subset as the main
benchmark, while retaining the full-holdout view as a robustness check.
Figures~\ref{fig:pcf-downstream-dist} and~\ref{fig:pcf-sector-tail} illustrate how sector
structure in the Carbon Catalogue concentrates much of that heavy tail in a handful of
industrial categories.

The chosen estimator is applied to the full Amazon catalog to assign $\mathrm{PCF}_i$ to
each product; items with insufficient metadata are excluded from downstream analysis.
...
We examine how NDCG@10 and AvgPCF@10 evolve as $\lambda$ increases, holding category and
model fixed.
...
\paragraph{Findings.}
A plateau then cliff pattern is visible in most model and category combinations, but the
location of the cliff varies considerably by model. For BPR and LightGCN the engagement
cliff is very late ($\lambda \approx 0.91$--$0.94$ in Electronics and Home,
$0.8$--$0.9$ in Sports), meaning NDCG@10 remains nearly flat while AvgPCF@10 falls
throughout almost the entire $\lambda$ grid. Under a 5\% NDCG loss budget, BPR achieves
86.3\%, 84.1\%, and 69.7\% carbon reduction in Electronics, Home \& Kitchen, and Sports
\& Outdoors respectively; LightGCN achieves 79.5\%, 83.7\%, and 74.4\%.

NeuMF is the exception. Its higher baseline engagement (NDCG@10 of 0.0618 in Electronics)
produces a peaked score distribution that re-ranking disrupts more readily: the cliff
arrives at $\lambda=0.3$ in Electronics (only 7.6\% reduction within the 5\% NDCG budget)
and at $\lambda=0.5$ in Sports \& Outdoors (43.8\%). Home \& Kitchen is the outlier,
NeuMF there actually \emph{gains} NDCG@10 at moderate $\lambda$ (cliff at $\lambda=0.96$),
reaching 81.1\% carbon reduction within budget. This suggests that in Home \& Kitchen the
re-ranker serendipitously elevates items that are both lower-carbon and higher-relevance,
consistent with the lower baseline AvgPCF@10 (106.7 kg) relative to BPR/LightGCN in the
same category (224.9 and 236.5 kg). Overall, Figures~\ref{fig:lambda-electronics}
through~\ref{fig:lambda-sports} answer RQ1 and RQ2 positively for BPR and LightGCN, and
partially for NeuMF outside Electronics.

%-----------------------------------------------------------------------
\subsection{Engagement and Carbon Pareto Frontiers}
\label{sec:pareto-results}
%-----------------------------------------------------------------------

The Pareto frontier directly characterises the achievable operating points: configurations
for which no other $\lambda$ value simultaneously achieves higher engagement and lower
carbon footprint.
...
\paragraph{Findings.}
Table~\ref{tab:pareto-summary} reports the best Pareto operating point per model under the
$\geq$10\% carbon reduction constraint. Several patterns stand out.

First, BPR and LightGCN achieve their best Pareto points at high $\lambda$ values
(0.60--0.70 for Electronics/Home; 0.075--0.10 for Sports) because their score
distributions are flat enough that the re-ranker can aggressively favour low-carbon items
without destroying relevance ordering. Electronics BPR at $\lambda=0.70$ delivers 73.2\%
carbon reduction while actually recording a marginal NDCG gain (0.0078 vs.\ baseline
0.0077).

Second, NeuMF's Pareto frontier is structurally different. In Electronics, the best
achievable $\geq$10\% point is $\lambda=0.50$ with only 10.0\% reduction (the engagement
cliff arrives well before any substantial carbon benefit is secured). In Home \& Kitchen
and Sports \& Outdoors, NeuMF's frontiers are more favourable (50.6\% and 10.9\% reduction
respectively), reflecting the later cliffs identified in the sensitivity analysis.

Third, Recall@10 closely tracks NDCG@10 at every Pareto-optimal point, confirming that
the re-ranker changes \emph{which} low-carbon items enter the top-10 without restructuring
the recall pool as a whole.
...
We overlay the Pareto frontiers of all three models within each category to identify which
recommendation backbone yields the best engagement and carbon trade-off, addressing RQ3.
...
\paragraph{Findings.}
NeuMF provides the strongest baseline engagement in all three categories: its $\lambda=0$
NDCG@10 is 0.0618 in Electronics, 0.0152 in Home and Kitchen, and 0.0102 in Sports and
Outdoors (substantially higher than LightGCN at 0.0248, 0.0065, and 0.0101, and BPR at
0.0077, 0.0035, and 0.0036). This advantage carries over to the $\geq$10\%
carbon-constrained operating points: NeuMF retains the highest absolute NDCG@10 across all
categories.

Absolute engagement level and carbon flexibility trade off in opposite directions. Because
NeuMF's score distribution is more peaked, its re-ranking frontier collapses earlier.
Within a 5\% NDCG loss budget, BPR leads in Electronics (86.3\%) and Home \& Kitchen
(84.1\%) while LightGCN leads in Sports \& Outdoors (74.4\%). NeuMF matches or leads only
in Home \& Kitchen (81.1\%), where re-ranking coincidentally improves NDCG. In Electronics,
NeuMF achieves only 7.6\% within the 5\% NDCG budget, an order of magnitude less than
BPR/LightGCN.

This directly answers RQ3: model choice determines both the level of engagement and the
flexibility of the carbon frontier. Practitioners targeting maximum absolute recommendation
quality should prefer NeuMF; those targeting maximum low-cost carbon reduction should
prefer BPR or LightGCN, accepting the lower baseline NDCG as a starting point.
...
We now fix the model and compare Pareto frontiers across the three product categories to
understand how category-level PCF distributions drive differences in the achievable
trade-off.
...
\paragraph{Findings.}
Category-level variation is substantial and interacts with model choice in non-obvious
ways. At the $\lambda=0$ baseline, Sports \& Outdoors has the highest AvgPCF@10 for BPR
(297.3 kg) and LightGCN (309.9 kg), reflecting heavier outdoor and sporting equipment in
those models' candidate pools. Electronics NeuMF, by contrast, has a surprisingly low
baseline AvgPCF@10 of only 58.1 kg, suggesting that NeuMF's stronger engagement model
steers users toward lighter electronic accessories rather than heavy appliances.

The heatmap (Figure~\ref{fig:carbon-heatmap}) summarises maximum carbon reduction within
a 5\% NDCG budget. Electronics BPR (86.3\%) and Home \& Kitchen BPR (84.1\%) are the most
carbon-flexible model and category combinations; Home \& Kitchen is notable in that
\emph{all three models} achieve above 80\% reduction (BPR 84.1\%, LightGCN 83.7\%, NeuMF
81.1\%), suggesting that the Home \& Kitchen item distribution offers rich low-carbon
substitutes at every model's engagement frontier. Electronics NeuMF (7.6\%) is the clear
outlier: its early engagement cliff leaves almost no usable carbon headroom within the
quality constraint.

These findings confirm a genuine model and category interaction: the achievable carbon
reduction is shaped jointly by the PCF distribution of the category's item pool and the
sharpness of the recommendation model's score distribution.
...
\hline
Electronics      & BPR      & 0.0078 & 0.0152 & 59.7  & 73.2\% & 0.70 \\
                 & LightGCN & 0.0249 & 0.0416 & 89.2  & 40.2\% & 0.25 \\
                 & NeuMF    & 0.0562 & 0.1065 & 52.3  & 10.0\% & 0.50 \\
\hline
Home \& Kitchen  & BPR      & 0.0035 & 0.0070 & 108.8 & 54.0\% & 0.60 \\
                 & LightGCN & 0.0066 & 0.0131 & 107.2 & 52.3\% & 0.60 \\
                 & NeuMF    & 0.0164 & 0.0351 & 52.8  & 50.6\% & 0.80 \\
\hline
Sports \& Outdoors & BPR    & 0.0036 & 0.0065 & 259.0 & 12.9\% & 0.075 \\
                 & LightGCN & 0.0101 & 0.0181 & 254.5 & 17.9\% & 0.10 \\
                 & NeuMF    & 0.0101 & 0.0212 & 165.9 & 10.9\% & 0.25 \\
\hline
\end{tabular}
...
NDCG@10 and Recall@10 are measured at $\lambda^*$; AvgPCF@10 is in
kg~CO$_2$e at $\lambda^*$; carbon reduction is relative to the $\lambda=0$
baseline. Baseline NDCG@10 values (not shown) are: Electronics 0.0077/0.0248/0.0618,
Home \& Kitchen 0.0035/0.0065/0.0152, Sports \& Outdoors 0.0036/0.0101/0.0102
for BPR/LightGCN/NeuMF respectively.}
...
absence of a review does not necessarily indicate negative feedback, creating challenges
due to \emph{missing negatives}. Review data may also be affected by \emph{temporal
...
Our approach relies on predicted product carbon footprints derived from metadata and
external PCF datasets. Although predictive models enable PCF estimation at catalog scale,
these estimates may contain significant uncertainty compared to full life-cycle assessments.
Errors in PCF estimation could propagate into the re-ranking stage, potentially affecting
the measured engagement and sustainability trade-offs.
...
estimation methods (for example through structured supply chain data or domain-specific
carbon models) could reduce uncertainty in sustainability signals. Third, integrating
...
Carbon Footprint labels are unavailable for most catalog items and must be inferred. We
proposed a modular two-stage framework: a retrieval-augmented PCF estimation pipeline that
combines few-shot LLM predictions with a nearest-neighbour fallback to transfer supervision
from a small set of life-cycle-assessed products to a large unlabeled e-commerce catalog,
followed by a post-hoc re-ranking strategy that blends model-predicted engagement with
estimated carbon footprint through a tunable parameter~$\lambda$.

We evaluated the framework on the Amazon Reviews dataset (1{,}000 users per category)
across three product categories and three recommendation models (BPR, NeuMF, LightGCN),
constructing engagement and carbon Pareto frontiers for each combination. The main finding
is that the achievable carbon reduction depends critically on which model is used. NeuMF
achieves the highest absolute engagement (NDCG@10 of 0.0618 in Electronics) but its peaked
score distribution creates an early engagement cliff at $\lambda\approx0.3$ in Electronics,
limiting carbon reduction within a 5\% NDCG budget to only 7.6\%. BPR and LightGCN have
flatter score distributions and very late cliffs ($\lambda\approx0.9$), enabling reductions
of 69--86\% within the same quality constraint across all three categories. Home \& Kitchen
is the most carbon-flexible category overall: all three models achieve above 80\% carbon
reduction within a 5\% NDCG budget, and NeuMF there actually gains NDCG at moderate
$\lambda$ values. These results suggest that post-hoc carbon-aware re-ranking can deliver
large sustainability gains for models with flat engagement distributions, while models with
strong score discrimination face a tighter trade-off and benefit from a smaller $\lambda$.
...
Act~\cite{dsa2022} and ensures that the trade-off between user engagement and environmental
impact remains open to inspection and deliberate adjustment by platform operators.

Several limitations should be noted. The framework relies on offline evaluation using
review-based implicit feedback, which introduces selection bias and missing negatives. PCF
estimates carry uncertainty relative to full life-cycle assessments, and errors in the
upstream estimation module propagate to the downstream re-ranking. Future work could
...
\hline
Electronics          & 1{,}000 & 3{,}006 & 6{,}410 & 6.4 \\
Home \& Kitchen      & 1{,}000 & 3{,}259 & 6{,}532 & 6.5 \\
Sports \& Outdoors   & 1{,}000 & 3{,}537 & 6{,}547 & 6.5 \\
\hline
\end{tabular}
\caption{Interaction statistics per category after 1{,}000-user sampling
