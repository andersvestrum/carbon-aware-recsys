...
\texttt{all-MiniLM-L6-v2} sentence encoder~\cite{reimers2019sbert} and retrieve the $k$ nearest
...
Sentence-Transformers model \texttt{all-MiniLM-L6-v2}~\cite{reimers2019sbert} and retrieve its five nearest Carbon
...
exactly the behavior that carbon-aware re-ranking changes. We do not emphasize Recall@10 in
the report because evaluation uses a leave-one-out test set with one held-out item per
user, so Recall@10 collapses to hit rate at 10 and is largely redundant with NDCG@10 while
being less sensitive to rank position.
...
over a 25-point grid spanning $[0,1]$. The sweep uses smaller increments near the extremes
...
The dominant pattern is a plateau followed by a cliff. For BPR and LightGCN, NDCG@10
...
The main takeaway is that substantial carbon savings are often available before
...
NeuMF provides the strongest absolute engagement in all three categories. Its
...
That advantage comes with less carbon flexibility. Because NeuMF's scores are more peaked,
its frontier collapses earlier under carbon-aware re-ranking, especially in Electronics. By
...
interpretable trade-off between recommendation quality and environmental impact. Across all
models and categories, we observe a consistent \emph{plateau--cliff} pattern: for a wide
range of $\lambda$, substantial reductions in recommended-item carbon footprint are achieved
with minimal loss in NDCG@10, followed by a sharp degradation once the carbon signal begins
to dominate relevance. This pattern has important implications for both the design and
deployment of sustainability-aware recommender systems.
...
Categories such as Home and Kitchen exhibit high substitutability, where many items provide
similar utility but differ in carbon footprint, enabling large reductions without impacting
relevance. In contrast, categories such as Electronics contain more functionally distinct
items, limiting the availability of low-carbon alternatives and leading to sharper
trade-offs. This interaction between category structure and model behavior implies that a
single global value of $\lambda$ is unlikely to be optimal in practice. Effective deployment
may instead require category-specific or dynamically adapted trade-off parameters.
...
Our findings align with prior work on sustainability-aware recommender systems, in
particular Kalisvaart et al.~\cite{kalisvaart2025greenerrec}, who show that carbon-aware
re-ranking can substantially reduce the environmental footprint of recommendations with
only modest losses in accuracy. We observe the same qualitative pattern, including a broad
region where carbon reductions can be achieved with minimal impact on engagement.

A key difference, however, lies in the availability of sustainability signals. Prior work
assumes access to item-level carbon labels, whereas our setting requires estimating PCF for
sampled Amazon catalog subsets totalling 72{,}000 products, with 24{,}000 products in each
of three categories. Our results therefore extend
these findings to a more realistic large-scale e-commerce scenario, demonstrating that the
engagement--sustainability trade-off persists even when environmental signals are incomplete
and noisy. This suggests that carbon-aware re-ranking is a robust paradigm that generalizes
beyond domains with curated sustainability data.

\paragraph{Implications for future systems.}
Taken together, these findings suggest that carbon-aware recommendation is both feasible
and practically relevant, but that its effectiveness depends on multiple interacting
factors: model architecture, category structure, and the availability of suitable
alternatives. Future work could extend this framework by integrating carbon objectives
directly into model training, exploring adaptive or personalized $\lambda$ strategies, or
incorporating richer signals of user preference and environmental impact. More broadly, the
results indicate that recommender systems can act as a scalable mechanism for influencing
consumption patterns, and that even simple re-ranking strategies can yield meaningful
environmental benefits when applied at scale.
